...
modelIR = keras.applications.inception_resnet_v2.InceptionResNetV2(include_top=True, weights=None, input_tensor=None, input_shape=(dimSz,dimSz,3), pooling=None, classes=12)
modelIR.summary()

modelIR.compile(optimizer ='adam', loss = 'categorical_crossentropy', metrics =['accuracy'])
...
modelName = 'incresnet2'
modelFileNm = modelName+str(dimSz)+'_'+str(epochSz)+'.json'

with open(expPath+modelFileNm, 'w') as f:  #>> Windows version
    f.write(modelIR.to_json())
...
bestWgFileNm = 'best_weights12_'+modelName+str(dimSz)+'_'+str(epochSz)+'.h5'
...
history = modelIR.fit_generator(training_set,
	steps_per_epoch = 20,
	epochs = 3,
...
finalWgFileNm = 'final_weights12_'+modelName+str(dimSz)+'_'+str(epochSz)+'.h5'

modelIR.save_weights(expPath+finalWgFileNm)
